...
export const AcceptBattleModal: React.FC<{ battle: SongBattle, onClose: () => void }> = ({ battle, onClose }) => {
    const { currentUserCollection, acceptBattle } = useContext(UserContext)!;
    ...
    const handleAccept = () => {
        ...
        acceptBattle(battle.id, selectedSongs);
        onClose();
    };
    ...
    return (
        <div className="modal-overlay" onClick={onClose}>
            <div className="modal-content w-full max-w-2xl bg-gray-800 rounded-lg p-6 flex flex-col" onClick={e => e.stopPropagation()}>
                <h3 className="text-2xl font-bold mb-1">Accept Challenge from <span className="text-indigo-400">{battle.challengerName}</span></h3>
                <p className="text-gray-400 mb-4">Select 5 songs from your collection to counter their deck.</p>
